...
{
    public class GenerateAnotherSpiky : MonoBehaviour
    {
        private Mesh mesh;
        ...
        class PointFunctionRoundHill : PointFunction
        {
            ...
            public override HeightAndColor calculateAtPoint(float x, float y, bool noFunctionYetIn, ref bool noFunctionYetOut)
            {
                ...
                {
                    height = heightHill * (2 - (Mathf.Tan(Mathf.Abs(xShifted)) + Mathf.Tan(Mathf.Abs(yShifted))));
                }
        ...
        private void Generate()
        {
            ...
            var textureRef = new Texture2D(sizeTexture * sizeGenTexture, sizeTexture * sizeGenTexture, TextureFormat.RGB24, false);
            textureRef.filterMode = FilterMode.Trilinear;
            ...
            for (int y = 0, i = 0; y <= sizeTexture * sizeGenTexture; y++)
            {
                for (int x = 0; x <= sizeTexture * sizeGenTexture; x++, i++)
                {
                    ...
                    Color whatToPlaceRGBf = Color.white;

                    int xRef = x / 10;
                    int yRef = y / 10;

                    if (xRef % 2 == 0 && yRef % 2 != 0)
                    {
                        whatToPlaceRGBf = Color.black;
                    }
                    else if (xRef % 2 != 0 && yRef % 2 == 0)
                    {
                        whatToPlaceRGBf = Color.black;
                    }
                    else if (xRef % 2 == 0 && yRef % 2 == 0)
                    {
                        whatToPlaceRGBf = Color.white;
                    }
                    textureRef.SetPixel(x, y, whatToPlaceRGBf);
                }
            }
            CreateImagePngFiles(textureRef, 0, "TextureRef");
        ...
        public Color getColorFromAltitude(float x, float y, float height, float maxHill, int withSize)
        {
            ...
            float blue = Mathf.Cos(height / withSize);
            float green = Mathf.Sin(height / withSize);

            Color newColor = new Color(0.2f, green, blue); 
