...
{
    public class GenerateGroundActualSquares : MonoBehaviour
    {
        private Mesh mesh;
        ...
        class PointFunctionRoundHill : PointFunction
        {
            ...
            public override void init(float x, float y, int type, float size, float height)
            {
                ...
                writer = new StreamWriter("function" + Mathf.CeilToInt(x) + "_" + Mathf.CeilToInt(y) + "_" + Mathf.CeilToInt(size) + "_" + Mathf.CeilToInt(height) + ".log", true);
            }
            ...
            public override HeightAndColor calculateAtPoint(float x, float y, bool noFunctionYetIn, ref bool noFunctionYetOut)
            {
                ...
                */
                if (xShifted < 0f && xShifted < -0.9f && xShifted > -1f)
                {
                    stepx = 10*(1f + xShifted);
                }
                else if (xShifted > 0f && xShifted > 0.9f && xShifted < 1f)
                {
                    stepx = 10 * (1f - xShifted);
                }
                else if (xShifted > -1f && xShifted < 1f)
                {
                    stepx = 1;
                }
                else
                ...
                if (yShifted < 0f && yShifted < -0.9f && yShifted > -1f)
                {
                    stepy = 10 * (1f + yShifted);
                }
                else if (yShifted > 0f && yShifted > 0.9f && yShifted < 1f)
                {
                    stepy = 10 * (1f - yShifted);
                }
                else if (yShifted > -1f && yShifted < 1f)
                {
                    stepy = 1;
                }
                else
                ...
                if (xShifted > -1 && yShifted > -1 && xShifted < 1 && yShifted < 1)
                {
                    writer.WriteLine(x + " . " + y + ": " + xShifted + " . " + yShifted + " : Using "+ stepx + " and " + stepy);
                    ourNewData.height = heightHill * stepx * stepy;
                }
                else
                {
                    ourNewData.height = 0;
                ...
                else if (noFunctionYetIn && (ourNewData.height < 5))
                {
        ...
        private void Generate()
        {
            ...
            Debug.Log("Generating the map took " + (Time.realtimeSinceStartup - currentTime));
            currentTime = Time.realtimeSinceStartup;
            ...
            Debug.Log("Generating the vertexes took " + (Time.realtimeSinceStartup - currentTime));
            currentTime = Time.realtimeSinceStartup;

            // Now the 8k texture
            clearFunctions();

            int sizeGenTexture = 1; // 2 for testing or heightmap
            ...
            generateHeightMap(pointInfo, sizeGenTexture, textureHeightMap, textureHeightMapInfo);

            Debug.Log("Generating the height map took " + (Time.realtimeSinceStartup - currentTime));

            currentTime = Time.realtimeSinceStartup;

            sizeGenTexture = 2; // 6 for first plan texture
            ...
            for (int y = 0, i = 0; y <= sizeTexture * sizeGenTexture; y++)
            {
                for (int x = 0; x <= sizeTexture * sizeGenTexture; x++, i++)
                {
                    ...
                    if (pointInfo.height < 8)
                    {
                        ...
                        Color grass = new Color(Random.value * 0.4f + 0.6f, Random.value * 0.2f , Random.value * 0.1f);
                        pointInfo.color = Color.Lerp(pointInfo.color, grass, 0.5f);
                    }
            ...
            Debug.Log("Generating the texture took " + (Time.realtimeSinceStartup - currentTime));
            currentTime = Time.realtimeSinceStartup;
            ...
            Debug.Log("Saving the maps took " + (Time.realtimeSinceStartup - currentTime));
            currentTime = Time.realtimeSinceStartup;
            ...
            Debug.Log("Setting the texture took " + (Time.realtimeSinceStartup - currentTime));
            currentTime = Time.realtimeSinceStartup;
        }

        private HeightAndColor generateHeightMap(HeightAndColor pointInfo, int sizeGenTexture, Texture2D textureHeightMap, Texture2D textureHeightMapInfo)
        {
            for (int y = 0, i = 0; y <= sizeTexture * sizeGenTexture; y++)
            {
                for (int x = 0; x <= sizeTexture * sizeGenTexture; x++, i++)
            ...
            for (int y = 0, i = 0; y <= sizeTexture * sizeGenTexture; y++)
            {
                for (int x = 0; x <= sizeTexture * sizeGenTexture; x++, i++)
                {
                    ...
                    if (false && noFunctionYet && x < maxSizeSmallHill && y < maxSizeSmallHill && Random.value > 0.9995f )
                    {
                        //Debug.Log("Addind a small hill!");
        ...
        public Color getColorFromAltitude(int x, int y, float height, float maxHill, int withSize)
        {
            int shiftX = (x % 80 + y) % 80; // 20 & 20 for small stripes of texture size 1
