...
{
    public class GenerateGroundBackgroundSquare : MonoBehaviour
    {
        private Mesh mesh;
        ...
        class PointFunctionRoundHill : PointFunction
        {
            ...
            public override HeightAndColor calculateAtPoint(float x, float y, bool noFunctionYetIn, ref bool noFunctionYetOut)
            {
                ...
                */
                stepx = Mathf.Ceil(xShifted * 10);
                stepy = Mathf.Ceil(yShifted * 10);
        ...
        private void Generate()
        {
            ...
            sizeGenTexture = 1; // 6 for first plan texture
        ...
        public Color getColorFromAltitude(float x, float y, float height, float maxHill, int withSize)
        {
            ...
            float xShifted = x/1000;
            float yShifted = y/1000; //  (y + yShift)/sizeHill; // Get cut...
            
            float stepx = Mathf.Ceil(xShifted * 10)/10;
            float stepy = Mathf.Ceil(yShifted * 10)/10;

            //height = 20f * Mathf.Cos(x / 10f) * Mathf.Cos(y / 10f);
            Color newColor = new Color(stepx+stepy, stepx+stepy, 1 - stepx + stepy);
