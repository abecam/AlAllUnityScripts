...
{
    public class GenerateCutRoundHills : MonoBehaviour
    {
        private Mesh mesh;
        ...
        class PointFunctionRoundHill : PointFunction
        {
            ...
            int cutDir = 0; // Which way do we cut the hill :)
            public override void init(float x, float y, int type, float size, float height)
            {
                ...
                cutDir = Mathf.CeilToInt(Random.value * 4);
            }
            ...
            public override HeightAndColor calculateAtPoint(float x, float y, bool noFunctionYetIn, ref bool noFunctionYetOut)
            {
                ...
                if (!((cutDir == 0 && xShifted < 0) || (cutDir == 1 && xShifted > 0) || (cutDir == 2 && yShifted < 0) || (cutDir == 3 && yShifted > 0)))
                {
                    height = heightHill * (1 - (Mathf.Pow(xShifted, 2) + Mathf.Pow(yShifted, 2)));
                }
        ...
        private void Generate()
        {
            ...
            sizeGenTexture = 4; // 6 for first plan texture
