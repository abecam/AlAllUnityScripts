...
{
    public class GenerateAnotherSphere : MonoBehaviour
    {
        private Mesh mesh;
        ...
        class PointFunctionRoundHill : PointFunction
        {
            ...
            public override HeightAndColor calculateAtPoint(float x, float y, bool noFunctionYetIn, ref bool noFunctionYetOut)
            {
                ...
                {
                    height = heightHill * (Mathf.Sqrt(4 - 4*Mathf.Pow(xShifted, 2) + 4*Mathf.Pow(yShifted, 2))); // Sqrt(4-x^2-y^2)
                }
                if (height < 0)
                {
                    height = 0;
        ...
        public Color getColorFromAltitude(float x, float y, float height, float maxHill, int withSize)
        {
            ...
            float blue = Mathf.Cos(x / (0.2f*((float )withSize)*height));
            float green = Mathf.Cos(y / (0.2f * ((float )withSize) *height));
