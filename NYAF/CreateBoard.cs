...
public class CreateBoard : MainApp
{
    ...
    void Start()
    {
        ...
        if (haskey)
        {
            typeOfSounds = LocalSave.GetInt("typeOfSounds");
        }
        loadSoundSynths();
        ...
        if (haskey)
        {
            isRotation = LocalSave.GetBool("isRotation");
        }

        // And save the current level so the continue can work
        LocalSave.SetInt("currentLevel", currentLevel);
        LocalSave.SetInt("lastMode", currentGameMode);
        LocalSave.Save();
    ...
    */
    ...
    {
        if ((currentGameMode != 2 && currentGameMode != 3) || !isRotation)
        {
            Vector3 worldPos = Camera.main.ScreenToWorldPoint(mousePos);
    ...
    private void GetRandomPresetForModeB(bool changeLevel = false)
    {
        ...
        while (!wasFound)
        {
            // The list is not very long, so we can brute-force our way here. Probably not so much slower than building an intermediate hashmap or anything.
            // But must change if the list start being longer!!!
            int newPos = (int )(Random.value * (float )listOfPresets.inLevel.Count);

            if (listOfPresets.inLevel[newPos] == currentLevel)
            {
                ...
                if (changeLevel)
                { 
                    Debug.Log("Placing in the same level, trial " + iTrials);
                }
                else
                {
                    Debug.Log("Placing initially, trial " + iTrials);
                }
            }
            else if (changeLevel)
            {
                int newLevel = listOfPresets.inLevel[newPos];

                Debug.Log("Placing in a different level, trial " + iTrials);

                // Go to the level found, hopefully seamlessly :) :)
                if (newLevel == 1)
                {
                ...
                {
                    UnityEngine.SceneManagement.SceneManager.LoadScene(rootNameOfLevel + newLevel);
                }
            ...
            if (iTrials > 50)
            {
                Debug.LogWarning("Could not find a preset for the current level (" + currentLevel + ")");
                wasFound = true;
            }
        }
    }
    ...
    {
        // Force load before
        LocalSave.HasBoolKey("levelReached1");
    ...
    */
    ...
    public void resetLevelReached()
    {
        ...
        LocalSave.SetInt("levelReached4", 1);
        LocalSave.Save();
    }
    ...
    {
        LocalSave.DeleteKey("currentLevel");
        LocalSave.Save();
    }

    public void deleteCoinsAndPurchases()
    {
        LocalSave.SetInt("TotalCoinsWon", 0);

        // Now to restore in the shop
        LocalSave.SetInt("Coins", 0);
    ...
    {
        LocalSave.SetInt("typeOfSounds", newTypeOfSound);
        LocalSave.Save();
    ...
    {
        LocalSave.SetBool("isRotation", newIsRotation);
        LocalSave.Save();
