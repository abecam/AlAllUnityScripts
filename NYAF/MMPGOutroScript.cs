...
public class MMPGOutroScript : MonoBehaviour
{
    private const string TIMETOFINISH = "TIMETOFINISH";
    string timeSpentTxt = "";
    ...
    public LocalizedString outroCongratulations;
    private string localizedCongratulationsText = "";
    ...
    public LocalizedString outroTimeFinish;
    private string localizedTimeFinishText = "";

    public LocalizedString outroNewBestTime;
    private string localizedNewBestTimeText = "";

    public LocalizedString outroBestTimeFinish;
    private string localizedBestTimeFinishText = "";

    void OnEnable()
    {
        outroCongratulations.StringChanged += UpdateStringCongratulations;
        ...
        outroTimeFinish.StringChanged += UpdateTimeFinish;
        outroNewBestTime.StringChanged += UpdateNewBestTime;
        outroBestTimeFinish.StringChanged += UpdateBestTimeFinish;
    }
    ...
    void UpdateStringCongratulations(string s)
    {
        localizedCongratulationsText = s;
    }
    ...
    void UpdateTimeFinish(string s)
    {
        localizedTimeFinishText = s;
    }

    void UpdateNewBestTime(string s)
    {
        localizedNewBestTimeText = s;
    }

    void UpdateBestTimeFinish(string s)
    {
        localizedBestTimeFinishText = s;
        SetCreditText();
    }
    ...
    private void SetCreditText()
    {
        ...
        {
           localizedCongratulationsText+ "\n"+localizedAGameByText+"\nAlain Becam",
           TIMETOFINISH,
           localizedGraphicsByTxt + localizedNobodyTxt + "\n"+localizedAndTxt+"\nSébastien Lesage\n" + localizedMonkBombTxt,
           localizedTestingGamePlayTxt + "\nWilliam",
           localizedMusicByTxt  + "\nAlexander Nakarada",
           localizedMusicByTxt  + "\nChan Redfield",
           localizedFontByTxt + "\nGlametrixBold by\nGluk\nwww.glukfonts.pl",
    ...
    private GameObject[] allBrotherText = new GameObject[20];
    private TextMesh[] allTextMesh = new TextMesh[20];
    private Transform[] allTextTransform = new Transform[20];

    private GameObject[] allBacks = new GameObject[NB_OF_CUBE_X_Y];

    int currentPlace = 0;

    const int nbFrameForPhase = 120;
    const int nbFrameForPhasePause = 160;

    int iTurn = 0;

    private const int NB_OF_ELEMENTS = 20;

    private const int NB_OF_CUBE_X = 10;
    private const int NB_OF_CUBE_Y = 10;
    private const int HALF_NB_OF_CUBE_X = NB_OF_CUBE_X / 2;
    private const int HALF_NB_OF_CUBE_Y = NB_OF_CUBE_Y / 2;

    private const int NB_OF_CUBE_X_Y = NB_OF_CUBE_X * NB_OF_CUBE_Y;
    ...
    void Start()
    {
        ...
        for (int iBrother = 0; iBrother < NB_OF_ELEMENTS; iBrother++)
        {
            ...
            allBrotherText[iBrother].transform.localScale = new Vector2(1, 1);

            allTextMesh[iBrother] = allBrotherText[iBrother].GetComponent<TextMesh>();
            allTextTransform[iBrother] = allBrotherText[iBrother].transform;
        }

        int iBrother2 = 0;

        for (int iBrotherX = 0; iBrotherX < NB_OF_CUBE_X; iBrotherX++)
        {
            for (int iBrotherY = 0; iBrotherY < NB_OF_CUBE_Y; iBrotherY++)
            {
                allBacks[iBrother2] = Instantiate(gameBack);

                allBacks[iBrother2].transform.parent = this.transform;

                allBacks[iBrother2].SetActive(true);

                allBacks[iBrother2].transform.localScale = new Vector3(0.5f, 0.5f, 0.5f);

                float floatX = ((float)iBrotherX - HALF_NB_OF_CUBE_X)/10;
                float floatY = ((float)iBrotherY - HALF_NB_OF_CUBE_Y)/10;

                allBacks[iBrother2].transform.localPosition = new Vector3(floatX*4, floatY*4, 5);

                Renderer oneRendered = allBacks[iBrother2].GetComponent<Renderer>();

                oneRendered.material.color = new Color(oneRendered.material.color.r, oneRendered.material.color.g, oneRendered.material.color.b, 0.6f);
    ...
    private void completeStrings()
    {
        ...
        string nbOfHours = ((hoursSpentMode > 0) ? hoursSpentMode + " hours\n" : "");
        string nbOfMins = ((hoursSpentMode > 0) || (minutesSpentMode > 0) ? minutesSpentMode + " minutes\n" : "");
        string nbOfSeconds = ((timeSpentLongMode > 0) ? timeSpentLongMode + " seconds\n" : "0 seconds");

        timeSpentTxt = localizedTimeFinishText.Replace("[HOURS]", "\n" + nbOfHours).Replace("[MINUTES]", "\n" + nbOfMins).Replace("[SECONDS]", "\n" + nbOfSeconds);
        ...
        if (allTimeSpent < bestTimeSpent)
        {
            ...
            timeSpentTxt += "\n"+ localizedNewBestTimeText;

            LocalSave.SetFloat("nypb_bestTime", bestTimeSpent);
            LocalSave.Save();
        }
        else
        {
            ...
            timeSpentTxt += "\n" + localizedBestTimeFinishText.Replace("[HOURS]", "\n" + bestTimeNbOfHours).Replace("[MINUTES]", "\n" + bestTimeNbOfMins).Replace("[SECONDS]", "\n" + bestTimeNbOfSeconds);
        }
        // You finished the mode in [HOURS] hours[MINUTES] minutes[SECONDS] seconds
        // Best time is [HOURS] hours [MINUTES]  minutes [SECONDS] seconds
    }

    const float stepScale = (1 - 0.05f) / ((float)nbFrameForPhase / 2);

    const float stepScaleBack = (10 - 0.01f) / ((float)nbFrameForPhase / 2);

    float lastSize = 10;

    int iFadingSteps = 0;
    const int nbOfFadingSteps = 90;
    const float stepFading = 1f / ((float)nbOfFadingSteps);
    private float sizeTextMin = 0.05f;
    ...
    void Update()
    {
        ...
        currentPlace++;
        if (currentPlace > nbFrameForPhase)
        {
            if (iFadingSteps < nbOfFadingSteps)
            {
                float newAlpha = allBrotherText[0].GetComponent<TextMesh>().color.a - stepFading;
                float newAlpha2 = allBacks[0].GetComponent<Renderer>().material.color.a - stepFading;

                for (int iBrother = 0; iBrother < NB_OF_ELEMENTS; ++iBrother)
                {
                    allTextMesh[iBrother].color = new Color(allTextMesh[iBrother].color.r, allTextMesh[iBrother].color.g, allTextMesh[iBrother].color.b, newAlpha);
                }
            ...
            {
                if (currentPlace > nbFrameForPhasePause)
                {
                    // After the title, we must zoom out the text much more
                    sizeTextMin = 0.035f;
                    ...
                    { 
                        ...
                        allTextMesh[iBrother].color = new Color(allTextMesh[iBrother].color.r, allTextMesh[iBrother].color.g, allTextMesh[iBrother].color.b, 1);

                        string lineOfText = allTitlesAndNames[iText];
                        if (lineOfText.Equals(TIMETOFINISH))
                        {
                            lineOfText = timeSpentTxt;
                        }
                        allTextMesh[iBrother].text = lineOfText;

                        allTextTransform[iBrother].localScale = new Vector2(1, 1);
                    }         

                    iText++;

                    if (iText >= allTitlesAndNames.Count)
                    {
                        iText = 0;
                    }
                }
        ...
        else
        {
            ...
            for (int iBrother = 0; iBrother < NB_OF_ELEMENTS; ++iBrother)
            {
                ...
                if (lastSize < sizeTextMin)
                {
                    lastSize = sizeTextMin;
                }
                allTextTransform[iBrother].localScale = new Vector2(lastSize, lastSize);
            }
        }

        {
            // Rotate the first cube and propagate to the other one
            allBacks[0].transform.Rotate(2 * Vector3.right * Time.deltaTime);
            allBacks[0].transform.Rotate(3 * Vector3.up * Time.deltaTime);
            ...
            for (int iBrotherX = 0; iBrotherX < NB_OF_CUBE_X; iBrotherX++)
            {
                for (int iBrotherY = 0; iBrotherY < NB_OF_CUBE_Y; iBrotherY++)
                {
                    if (iBrother != 0)
                    {
                        Quaternion localRotationPreviousCube = allBacks[iBrother - 1].transform.localRotation;

                        allBacks[iBrother].transform.localRotation = new Quaternion(localRotationPreviousCube.x,
                            localRotationPreviousCube.y, localRotationPreviousCube.z, localRotationPreviousCube.w);
                    }
                    iBrother++;
                }
            }
    ...
    private void createTexture()
    {
        ...
        Renderer imageRenderer = gameBack.GetComponent<Renderer>();
        imageRenderer.material.mainTexture = texture;

        // Set it for all copies
        for (int iBrother = 0; iBrother < NB_OF_CUBE_X_Y; iBrother++)
        {
            Renderer oneRenderer = allBacks[iBrother].GetComponent<Renderer>();

            oneRenderer.material.mainTexture = texture;
        }
    }
    ...
    private void prepareMap()
    {
        ...
        int currentLevel = (int)(Random.value * 200);
