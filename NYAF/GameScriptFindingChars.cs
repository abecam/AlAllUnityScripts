...
public class GameScriptFindingChars : MainApp
{
    ...
    public PlayScript playScript; // We need our playscript to be able to shake the player (at least)
    ...
    void Start()
    {
        ...
        if (haskey)
        {
            pieceTransparences = LocalSave.GetInt("FindingNYAF_transparentChars") == 1;
        ...
        if (haskey)
        {
            showHints = LocalSave.GetInt("FindingNYAF_showHints") == 1;
        }

        haskey = LocalSave.HasIntKey("FindingNYAF_showArrows");
        if (haskey)
        {
            showArrows = LocalSave.GetInt("FindingNYAF_showArrows") == 1;
        }

        // Which sounds do we play
        haskey = LocalSave.HasIntKey("FindingNYAF_typeOfSounds");
        if (haskey)
        {
            typeOfSounds = LocalSave.GetInt("FindingNYAF_typeOfSounds");
        }
        loadSoundSynths();

        // Check how far we are
        haskey = LocalSave.HasIntKey("FindingNYAF_modeFinished");
        if (haskey)
        {
            maxGameMode = LocalSave.GetInt("FindingNYAF_modeFinished");
        }

        haskey = LocalSave.HasBoolKey("FindingNYAF_isRotation");
        if (haskey)
        {
            isRotation = LocalSave.GetBool("FindingNYAF_isRotation");
        }
    ...
    */
    ...
    {
        return Application.persistentDataPath + "/" + getCurrentMode() + "finding_gamesave" + currentLevel + ".save";
    }
        ...
        if (Physics.Raycast(ray, out hit, 100))
        {
            ...
            foreach (Transform childCharacter in characters.transform)
            {
                if (childCharacter == hit.transform)
                {
                    ...
                    // Parc the object
                    childCharacter.position = new Vector3(400, 0, -9);
        ...
        foreach (Transform childCharacter in characters.transform)
        {
            if (childCharacter.position.x < 90)
            {
                ...
                if (showHints)
                {
                    ...
                    MoveInWater movingScript = aNewHint.GetComponent<MoveInWater>();
    ...
    void loadNext()
    {
        ...
        LocalSave.SetFloat("FindingNYAF_timeSpentLevel" + currentGameMode + "-" + currentLevel, 0);
        ...
        if (currentLevel == 11 && (currentGameMode == 0 || currentGameMode >= 2))
        {
            ...
            ourShowResult.showResultText(hoursSpent, minutesSpent, timeSpentLong, hoursSpentMode, minutesSpentMode, timeSpentLongMode, "OutroFindingNyaf");
        }
        else
        {
            ...
            LocalSave.SetInt("FindingNYAF_currentLevel", currentLevel++);
            LocalSave.Save();

            ourShowResult.showResultText(hoursSpent, minutesSpent, timeSpentLong, hoursSpentMode, minutesSpentMode, timeSpentLongMode, "FindingNYAFINGS/MainScene");
        }
    }
    ...
    public void setUseTransparence(bool showTrans)
    {
        ...
        LocalSave.SetInt("FindingNYAF_transparentChars", pieceTransparences ? 1 : 0);
        LocalSave.Save();
    }
    ...
    public void setShowHints(bool newShowHints)
    {
        ...
        LocalSave.SetInt("FindingNYAF_showHints", newShowHints ? 1 : 0);
        LocalSave.Save();
    }
    ...
    public void setShowArrows(bool newShowArrows)
    {
        ...
        LocalSave.SetInt("FindingNYAF_showArrows", newShowArrows ? 1 : 0);
        LocalSave.Save();
    }

    public void setNextLevelReached()
    {
        bool haskey = LocalSave.HasIntKey("FindingNYAF_levelReached" + getCurrentMode());
        int levelReached = 0;
        if (haskey)
        {
            levelReached = LocalSave.GetInt("FindingNYAF_levelReached" + getCurrentMode());
        }

        if (currentLevel >= levelReached)
        {
            LocalSave.SetInt("FindingNYAF_levelReached" + getCurrentMode(), currentLevel + 1);
            LocalSave.Save();
        }
    ...
    {
        LocalSave.SetInt("FindingNYAF_levelReached1", 11);
        LocalSave.Save();
    }
    ...
    */
    internal string getCurrentModeFromSave()
    {
        ...
        bool haskey = LocalSave.HasIntKey("FindingNYAF_currentMode");
        if (haskey)
        {
            currentGameModeFromSave = LocalSave.GetInt("FindingNYAF_currentMode");
        }
    ...
    public void setCurrentModeNb(int newMode)
    {
        ...
        LocalSave.SetInt("FindingNYAF_currentMode", newMode);
        LocalSave.Save();
    }

    public void resetLevelReached()
    {
        LocalSave.SetInt("FindingNYAF_levelReached", 1);
        LocalSave.Save();

        LocalSave.SetInt("FindingNYAF_levelReached1", 1);
        LocalSave.Save();

        LocalSave.SetInt("FindingNYAF_levelReached2", 1);
        LocalSave.Save();

        LocalSave.SetInt("FindingNYAF_levelReached3", 1);
        LocalSave.Save();
    }

    public void resetModeReached()
    {
        LocalSave.SetInt("FindingNYAF_modeFinished", 0);
        LocalSave.Save();

        LocalSave.SetInt("FindingNYAF_currentMode", 0);
        LocalSave.Save();
    }
    ...
    public void resetTimeSpent()
    {
        ...
        {
            LocalSave.DeleteKey("FindingNYAF_bestTimeSpent" + iMode);
            LocalSave.SetInt("FindingNYAF__allTimeSpent" + iMode, 0);
            for (int iLevel = 1; iLevel < 12; iLevel++)
            {
                LocalSave.SetFloat("FindingNYAF_timeSpentLevel" + iMode + "-" + iLevel, 0);
            }
        }
        LocalSave.DeleteKey("FindingNYAF_bestScoreMode3");
    ...
    {
        bool haskey = LocalSave.HasIntKey("FindingNYAF_modeFinished");
        int maxModeReached = 0;
        if (haskey)
        {
            maxModeReached = LocalSave.GetInt("FindingNYAF_modeFinished");
        }

        LocalSave.SetInt("FindingNYAF_lastModeFinished", currentGameMode);
        ...
        {
            LocalSave.SetInt("FindingNYAF_modeFinished", currentGameMode + 1);
            LocalSave.Save();
    ...
    {
        bool haskey = LocalSave.HasIntKey("FindingNYAF_smallFound");
        int allSmallFoundFlag = 0;
        if (haskey)
        {
            allSmallFoundFlag = LocalSave.GetInt("FindingNYAF_smallFound");
        }
        ...
        LocalSave.SetInt("FindingNYAF_smallFound", allSmallFoundFlag);
        LocalSave.Save();
    ...
    {
        LocalSave.SetInt("FindingNYAF_smallFound", 0);
        LocalSave.Save();
    }
    ...
    internal static void deleteLevelSaves()
    {
        ...
        for (int iMode = 0; iMode < 10; iMode++)
        {
            ...
            {
                String pathToDelete = Application.persistentDataPath + "/FindingNYAF_" + iMode + "gamesave" + iLevel + ".save";

                if (iMode == 0)
                {
                    pathToDelete = Application.persistentDataPath + "/FindingNYAF_gamesave" + iLevel + ".save";
                }
    ...
    {
        LocalSave.SetInt("FindingNYAF_typeOfSounds", newTypeOfSound);
        LocalSave.Save();
    ...
    {
        LocalSave.SetBool("FindingNYAF_isRotation", newIsRotation);
        LocalSave.Save();
